...
def mandelbrot(xmin, xmax, ymin, ymax, Nx, Ny, escape_time):
    ...
    dx = abs((xmax-xmin)/Nx)
    dy = abs((ymax-ymin)/Ny) #Step lengths for values (resolution of grid)
    for i in range(Nx):
        for j in range(Ny):
...
if __name__=="__main__":
    ...
    xl, yl, pic = mandelbrot(xmin, xmax, ymin, ymax, Nx, Ny, escape_time)
    ...
    tm.test_init_rect_inside(xmin, xmax, ymin, ymax, escape_time, pic)

    plt.contourf(xl, yl, pic, cmap=plt.cm.bone)
    plt.colorbar()
    plt.show()
...
steinn@steinn-VirtualBox:~/Desktop/INF3331-steinnhm/assignment4$ python3 mandelbrot_3.py 2000 100
Took 2.47 seconds to calculate for 4000000 grid mesh points.
"""
